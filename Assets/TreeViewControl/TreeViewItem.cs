...
public class TreeViewItem
{
    ...
    {
		Rect lastRect;
		int offset = ParentControl.m_textureGuide.width + ParentControl.m_textureNormalChecked.width;
        if (null == ParentControl)
        {
            return;
        }
        ...
        if (!string.IsNullOrEmpty(Header))
        {
            ...
            if (ParentControl.IsHoverEnabled)
            {
				...
				if (size.x+(levels*offset)+15 > ParentControl.maxWidth) ParentControl.maxWidth = (int)size.x+(levels*offset)+15;
